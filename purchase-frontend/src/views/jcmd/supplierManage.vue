        ...
        <el-table :data="dataList" style="width: 100%">
          ...
          <el-table-column prop="reviewStatus" label="审核状态" width="120" show-overflow-tooltip>
            <template slot-scope="scope">
              <span>{{ scope.row.reviewStatus == 0 ? '已审核' : '进行中' }}</span>
            </template>
          </el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleEdit(index, item) {
      ...
      this.$confirm("供应商信息是否审核完成，请确认！", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(() => {
          ...
          this.$http.post("/supplier/reviewData?id=" + item.id)
            .then((res) =>{
              if (res.data.data === '0') {
                // 表示数据不能被删除
                this.$message({
    ...
    getDataList() {
      ...
      //后端请求
      this.$http.get("/supplier/listNoReview", params).then((res) => {

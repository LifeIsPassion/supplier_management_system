...
export default {
  ...
  methods: {
    ...
    openDialog() {
      ...
      this.dataDialogForm.sendStatus = '';
    },
    //关闭窗口
    closeDialog() {
      // 清空添加数据的表单
    ...
    handleSend(index,item) {
      if (item.sendStatus == 0 ){
        this.$confirm("该商品已发货","提示",{
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning",
        })
      ...
      this.$confirm("是否确认发货完成，请确认！", "提示", {
        ...
        .then(() => {
          ...
          this.dataDialogForm.id = item.id;
          this.$http.post("/purchase/send", this.dataDialogForm)
            .then((res) =>{
              if (res.data.data === '0') {
                // 表示数据不能被删除
                this.$message({
...
//---------------退货
    ...
    getDataList() {
      ...
      //后端请求 分页获取对象
      this.$http.get("/purchase/listHead", params).then((res) => {
        //需要通过响应的结果配置

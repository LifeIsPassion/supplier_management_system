    ...
    <el-card class=" box-card">
      <div>
        ...
        <el-table ref="multipleTable" :data="dataList" tooltip-effect="dark" style="width: 100%">
          ...
          <el-table-column prop="number" label="销售单号" width="100">
          </el-table-column>
          <el-table-column prop="exitNumber" label="退货单号" width="100">
          </el-table-column>
          <el-table-column prop="num" label="退购数量" width="80">
          </el-table-column>
          <el-table-column prop="price" label="退购单价" width="80">
          </el-table-column>
          <el-table-column prop="totalPrice" label="退购总价" width="80">
          </el-table-column>
          <el-table-column prop="time" label="退货时间" width="170">
            <template slot-scope="scope">
              <span>{{scope.row.time ==null ? '': scope.row.time.replace("T"," ") }}</span>
            </template>
          </el-table-column>
          <el-table-column prop="reson" label="退采原因" width="280">
          </el-table-column>
      ...
      <!-- Form -->
      <el-dialog :title="dataDialogForm.id === 0 ? '新增销售退货单' : '更新销售退货单'" width="35%" :visible.sync="dialogFormVisible"
        @close="closeDialog()">
        <el-form :model="dataDialogForm" :rules="rules" ref="purchaseExitForm">
          <el-form-item label="销售编号" label-width="120px" prop="number">
            ...
            <template slot-scope="{ item }">
              ...
              <span class="addr">{{ item.customer }}</span>
            </template>
          </el-autocomplete>
          </el-form-item>

          <el-form-item label="退销数量" label-width="120px" prop="num">
            <el-input v-model="dataDialogForm.num" placeholder="销售数量" style="width: 300px"></el-input>
          </el-form-item>
         <el-form-item label="退销单价" label-width="120px" prop="price">
          ...
          <el-form-item label="退采原因" label-width="120px" prop="reason">
...
export default {
  ...
  methods: {
    ...
    openDialog() {
      ...
      this.dataDialogForm.number = "";
      this.dataDialogForm.exitNumber = "",
        this.dataDialogForm.num = "",
        this.dataDialogForm.price = "",
        this.dataDialogForm.time = "",
        this.dataDialogForm.status = "",
        this.dataDialogForm.reason = "",
    ...
    update(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          this.$http.post("/saleExit/add", this.dataDialogForm)
            .then((res) => {
    ...
    handleDelete(index, item) {
      ...
      this.$confirm("此操作将永久该记录, 是否继续?", "提示", {
        ...
        .then(() => {
          ...
          this.dialogFormSubmitVisible = true;
          this.$http
            .post("/saleExit/delete/?id=" + item.id)
            .then((res) => {
              // console.log(res)
              if (res.data.data === '0') {
                // 表示数据不能被删除
    ...
    getDataList() {
      ...
      //后端请求 分页获取对象
      this.$http.get("/saleExit/list", params).then((res) => {

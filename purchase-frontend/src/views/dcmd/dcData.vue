...
<!--        <el-input class="Titledisabled" v-model="detail.title"></el-input>-->
        <div style="padding-top: 20px;padding-left: 10px">
          <h>采购清单excel表</h>
          <el-button type="primary" @click="exportExcel" style="margin-left:60px">导出</el-button>
        </div>
        <div style="padding-top: 40px;padding-left: 10px">
          <h>退货清单excel表</h>
          <el-button type="primary" @click="exportPurchaseExit()" style="margin-left:60px">导出</el-button>
        </div>
        <div style="padding-top: 60px;padding-left: 10px">
          <h>库存清单excel表</h>
          <el-button type="primary" @click="exportDeList()" style="margin-left:60px">导出</el-button>
        </div>
<!--        <div style="padding-top: 75px;padding-left: 10px">-->
<!--          <h>历史出库excel表</h>-->
<!--          <el-button type="primary" @click="submitNote()" style="margin-left:60px">导出</el-button>-->
<!--        </div>-->
        <!-- 富文本 -->
...
export default {
  ...
  methods: {
    exportDeList(){
      axios({
        url: '/dcData/exportDeList', // 后端提供的导出接口
        method: 'GET',
        responseType: 'blob', // 重要，设置响应类型为blob
      }).then((response) => {
        // 创建一个Blob对象，使用响应数据
        const blob = new Blob([response.data], { type: 'application/vnd.openxmlformats-officedocument.sheet' });
    ...
    exportPurchaseExit(){
      axios({
      url: '/dcData/exportPurchaseExit', // 后端提供的导出接口
      method: 'GET',
      responseType: 'blob', // 重要，设置响应类型为blob
    }).then((response) => {
      // 创建一个Blob对象，使用响应数据
    ...
    exportExcel() {
      axios({
        url: '/dcData/exportPurchase', // 后端提供的导出接口
        method: 'GET',
        responseType: 'blob', // 重要，设置响应类型为blob
      }).then((response) => {
        // 创建一个Blob对象，使用响应数据
    ...
    exp () {
      this.openLoading()
      window.open("http://localhost:8083/purchase/test")
      //this.$http.get("dcData/export").then((res) => {})
    },
    submitNote() {
      //请求参数封装
      const params = {

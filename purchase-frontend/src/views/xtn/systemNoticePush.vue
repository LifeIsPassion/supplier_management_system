<template>
  ...
  <el-row :gutter="20">
    ...
    <el-col :span="14">
      <el-row>
        <div style="margin-top:33px">
          <el-input class="selectNote" v-model="dataForm.select" placeholder="请输入公告名称" clearable></el-input>
          <el-button type="primary" icon="el-icon-search" @click="getNoteDateList"
                     style="margin-left: 5px;">查询</el-button>
        </div>
        <span style="margin-left:13px;">当前公告名称</span>
    ...
    <!-- 新建笔记------弹框 -->
    <div>
      <el-dialog title="请输入公告名称" :visible.sync="dialogNoteFormVisible">
        <el-form :model="tableNoteDialog">
          <el-form-item label="公告命名" label-width="200px">
...
export default {
  ...
  methods: {
    ...
    getNoteList() {
      ...
      //后端请求 分页获取对象
      this.$http.get("/bjNote/listSysByName").then((res) => {
        // console.log("笔记", res);
    ...
    getNoteDateList() {
      ...
      this.$http.get("/bjNote/listSysByName", params).then((res) => {
    ...
    dialogNoteForm() {
      ...
      this.$http.post("/bjNote/createSysByTitle?title="+title).then((res) => {
        // console.log(res);
